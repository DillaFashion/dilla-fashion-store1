  ...
  try {
    ...
    if (body.message) {
      ...
      if (text === "/start") {
        ...
        for (const p of products) await sendProduct(chatId, p);
      } else {
      ...
      if(body.message.location){
        ...
        await sendMessage(OWNER_CHAT_ID, `📍 Location from @${body.message.from.username || body.message.from.first_name}: Lat ${loc.latitude}, Lng ${loc.longitude}`);
        await sendMessage(chatId, "✅ Location received. We'll process your order!");
      }
      ...
      if (data.startsWith("like_")) {
        ...
        await sendMessage(chatId, `✅ Order received for ${product.name}. Please share your location.`);
        ...
        await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
          ...
          body: JSON.stringify({ chat_id: chatId, text: "Please share your location for delivery:", reply_markup: keyboard })
        });
      }
    }
